...
# Relative fitness in response to experimental evolution
...
# Answer = Yes: t = 11.471, df = 3.1867, p-value = 0.001068
# Minimization led to 53 % reduction in fitness.
...
# After evolution, minimization has 28% reduction in fitness
...
# All delta fitness are highly significant except for non-minimal ancestor vs. minimal evolved

#       Estimate   Std Error t value Pr(>|t|)    
# m.m    0.52834    0.04178  12.647 4.92e-07 ***
# m.nm  -0.00230    0.04178  -0.055    0.957    
# nm.nm  0.37889    0.04178   9.069 8.02e-06 ***
...
# Relevant test = `compnm.nm`: fitness differences for minimal vs. non-minimal 

#         Estimate   Std Error t value Pr(>|t|)    
# (Int).   0.52834    0.04178  12.647 4.92e-07 ***
# m.nm    -0.53064    0.05908  -8.982 8.68e-06 ***
# nm.nm   -0.14945    0.05908  -2.530   0.0323 *  
...
# Create figure for relative finess (Fig. 2)
...
# Create figure for delta relative fitness (Fig. S2)

```{r}
png(filename="./../../../figures/4.figshare/delta_relfitness.png",
    width = 1200, height = 1200, res = 96*2) 

par(mar = c(8, 7, 5, 7))

delta.fig <- plot(jitter(rep(1, length(delta.m.nm)), amount = 0.1), 
      delta.m.nm, ylim = c(-0.25, 1.0), xlim = c(0.5, 3.5), pch = 21, 
...
# Add x-axis labels
text(x = 1, y = -0.35, label = "min-nonmin", col = "grey", srt = 90, 
     xpd = NA, cex = 0.85, adj = 1)
text(x = 2, y = -0.35, label = "min-min", col = "grey", srt = 90, 
     xpd = NA, cex = 0.85, adj = 1)
text(x = 3, y = -0.35, label = "nonmin-nonmin", col = "grey", srt = 90, 
     xpd = NA, cex = 0.85, adj = 1)
...
# Add P-values
text(x = 1, y = -0.15, expression(~ italic("P")~ "= 0.96"), cex = 0.95, col = "grey")
text(x = 2, y = -0.15, expression(~ italic("P")~ "< 0.0001"), cex = 0.95, col = "grey")
text(x = 3, y = -0.15, expression(~ italic("P")~ "< 0.0001"), cex = 0.95, col = "grey")

# Add comparison of interaction terms
segments(2, 0.7, 3, 0.7, col = "grey", lwd = 1.25, xpd = NA)
segments(2, 0.65, 2, 0.7, col = "grey", lwd = 1.25, xpd = NA)
segments(3, 0.63, 3, 0.7, col = "grey", lwd = 1.25, xpd = NA)
text(x = 2.5, y = 0.75, expression(~ italic("P")~ "= 0.032"), cex = 0.95, col = "grey")
...
# Show Plot
img <- readPNG("./../../../figures/4.figshare/delta_relfitness.png")
