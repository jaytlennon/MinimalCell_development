...
## Roy: Section to be removed?????

```{r}
# ROY: this looks sketchy. Where are these numbers coming from below? 
# They appear to have been manually entered, which is error-prone and not reproducible.
# Please fix

# Since this isn't in our paper, as we've discussed, maybe this can be moved to a
# separate document of "extra analyses", whcih may certainly want or need to revisit
# at another time
...
img.non.min <- readPNG("figures/size.images/nonmin_04_014_scale2.png")
img.min <- readPNG("figures/size.images/min_11_010.png")

grid.newpage()
grid.raster(img.plot, x = 0.4, y = 0.5, width = 0.83)
grid.raster(img.non.min, x = 0.78, y = 0.66, width = 0.25)
grid.raster(img.min, x = 0.78, y = 0.395, width = 0.25)

grid.text("Non-minimal", x = 0.825, y = 0.555, 
            gp = gpar(fontsize = 12, col = "blue"))

grid.text("Minimal", x = 0.85, y = 0.29, 
            gp = gpar(fontsize = 12, col = "red"))

grid.polyline(x=c(0.82, 0.883),
             y=c(0.76, 0.76),
             gp=gpar(col= "white", lwd = 1.5))

grid.text(expression("1" ~ mu~"m"), x = 0.85, y = 0.74, 
...
grid.rect(unit(0.75, "npc"), unit(0.65, "npc"),
          width = unit(0.1, "npc"), height = unit(0.1, "npc"), default.units = "npc",
          gp=gpar(col = "white"))
