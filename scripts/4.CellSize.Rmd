...
# Difference in cell size between wt and minimal at ancestal timepoint?
# Problem is that n = 2 for wt and n = 1 for minimal. 
# So, I'm running a one sample t-test here, which probably has very low power
# Results: t = 4.9552, df = 1, p-value = 0.1268

anc.reps <- filter(size.reps, timepoint == "Ancestor") %>%
   group_by(strain, sub.strain)

anc.t.test <- t.test(anc.reps$mean.vol[2:3], mu = anc.reps$mean.vol[1])
...
# Difference in cell size between minimal and wt over time?
# Answer = Yes: t = 4.2253, df = 3.3248, p-value = 0.01971

wt.size  <- filter(evo.reps, strain == "Wildtype") %>%
  dplyr::select(strain, sub.strain, mean.vol)
wt.delt.size <- wt.size$mean.vol - mean(anc.reps$mean.vol[2:3])

min.size  <- filter(evo.reps, strain == "Minimal") %>%
  dplyr::select(strain, sub.strain, mean.vol)
min.delt.size <- min.size$mean.vol - mean(anc.reps$mean.vol[1])

delta.size <- data.frame(wt.delt.size, min.delt.size )
colnames(delta.size) <-c("Wildtype", "Minimal")

# Test for equal variance: P = 0.03905 --> non-homogeneous variance
equal.var.delta <- var.test(delta.size$Wildtype, delta.size$Minimal)

test.delta.size <- t.test(delta.size$Wildtype, delta.size$Minimal, var.equal = F)
```
