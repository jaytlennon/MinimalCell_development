...
do
	...
	echo "#$ -l vmem=50gb walltime=2:00:00" >> Mm_NSE_${i}_qc_clean.sh
	echo "" >> Mm_NSE_${i}_qc_clean.sh
	...
	echo "" >> Mm_NSE_${i}_qc_clean.sh
	echo "time cutadapt -u 17 -o Sample_${i}_R1_frontaxe.fastq Sample_${i}_R1.fastq" >> Mm_NSE_${i}_qc_clean.sh
	echo "" >> Mm_NSE_${i}_qc_clean.sh
	echo "time cutadapt -u -5 -o Sample_${i}_R1_backaxe.fastq Sample_${i}_R1_frontaxe.fastq" >> Mm_NSE_${i}_qc_clean.sh
	echo "" >> Mm_NSE_${i}_qc_clean.sh
	echo "time cutadapt -a AGATCGGAAGAGC -o Sample_${i}_R1_trimmed.fastq Sample_${i}_R1_backaxe.fastq" >> Mm_NSE_${i}_qc_clean.sh
	echo "" >> Mm_NSE_${i}_qc_clean.sh
...
#### Assembly ####
	...
	echo "#$ -l vmem=50gb walltime=24:00:00 " >> Mm_NSE_${i}_Assemble.sh
	echo "" >> Mm_NSE_${i}_Assemble.sh
	...
	echo "time bwa mem /N/dc2/projects/muri2/roy/Mm_NSE/reference/JCVI-syn3A_reference_CP016816.2/Synthetic.bacterium_JCVI-Syn3A.fasta Sample_${i}_R1_trimmed.fastq > Sample_${i}.sam" >> Mm_NSE_${i}_Assemble.sh
   	echo "" >> Mm_NSE_${i}_Assemble.sh
	echo "qsub -l walltime=8:00:00,vmem=20gb Mm_NSE_${i}_compress.fastq.sh" >> Mm_NSE_${i}_Assemble.sh
	echo "" >> Mm_NSE_${i}_Assemble.sh
