...
# Load data
```{r}
GC <- read.csv("../data/MinimalCell_TwoPoint_Mod.csv")

# Change times to numeric
GC$Time <- as.numeric(sub("^(\\d+):(\\d+).*", "\\1.\\2", GC$Time))
head(GC, header = T)

# Pull out wells with minimal vs. non-minimal cells

#non.min.A <- GC[3:75, c(1,2,6,10,14,18,22)]
non.min.A <- GC[3:which(GC[,1] == 15.00), c(1,2,6,10,14,18,22)]
min.A <- GC[3:nrow(GC), c(1,3,7,11,15,19,23)]
non.min.E <- GC[3:which(GC[,1] == 15.00), c(1,4,8,12,16,20,24)]
min.E <- GC[3:nrow(GC), c(1,5,9,13,17,21,25)]
```
