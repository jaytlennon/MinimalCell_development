...
# NM: replicate population 1  (plate 1)
...
# Min value in each column for blanking
# min <- nm.b1[,1:ncol(nm.b1)] %>% summarise_all(funs(min))

# nm.b1 <- as.data.frame(sweep(as.matrix(nm.b1[,1:ncol(nm.b1)]), 2, as.matrix(min)))
...
nm.240 <-  nm.b1[,c(1,9,19,29)]
nm.260 <-  nm.b1[,c(1,39,48,57)]
nm.280 <-  nm.b1[,c(1,10,20,30)]
nm.300 <-  nm.b1[,c(1,40,49,58)]
...
# NM: replicate population 2  (plate 2)
...
# Min value in each column for blanking
#min <- nm.b2[,1:ncol(nm.b2)] %>% summarise_all(funs(min))

#nm.b2 <- as.data.frame(sweep(as.matrix(nm.b1[,1:ncol(nm.b1)]), 2, as.matrix(min)))
...
# NM: replicate population 3  (plate 3)
...
nm.60 <-  nm.b3[,c(1,5,15,25)]
nm.80 <-  nm.b3[,c(1,35,44,53)]
nm.100 <-  nm.b3[,c(1,6,16,26)]
nm.120 <-  nm.b3[,c(1,36,45,54)]
...
# NM: replicate population 4  (plate 4)
...
# M: replicate population 1  (plate 1)
...
# M: replicate population 2  (plate 2; T40 didn't grow)
...
# Pull out wells over time
m.0 <-  m.b2[,c(1,2,11,20)]
m.10 <-  m.b2[,c(1,29,38,47)]
m.20 <-  m.b2[,c(1,3,12,21)]
m.30 <-  m.b2[,c(1,30,39,48)]
m.50 <-  m.b2[,c(1,31,40,49)]
m.60 <-  m.b2[,c(1,4,13,22)]
m.80 <-  m.b2[,c(1,32,41,50)]
m.100 <-  m.b2[,c(1,5,14,23)]
m.120 <-  m.b2[,c(1,33,42,51)]
m.140 <-  m.b2[,c(1,6,15,24)]
m.160 <-  m.b2[,c(1,34,43,52)]
m.200 <-  m.b2[,c(1,7,16,25)]
m.220 <-  m.b2[,c(1,35,44,53)]
m.240 <-  m.b2[,c(1,8,17,26)]
m.260 <-  m.b2[,c(1,36,45,54)]
m.280 <-  m.b2[,c(1,9,18,27)]
#m.300 <-  m.b2[,c(1,37,46,55)]
m.300 <-  m.b2[1:which(m.b2[,1] == 15.00), c(1,37,46,55)]
...
treats.m.2 <- read.csv("../data/treatments_batch2_M.csv")
treats.m.2[rep(seq_len(nrow(treats.m.2)), each = 18), ]

# M: RERUN: replicate population 2  (plate 2)

m.b2rr <- read.csv("../data/batch_M_2_rerun.csv")

# Change times to numeric
m.b2rr$Time <- as.numeric(sub("^(\\d+):(\\d+).*", "\\1.\\2", m.b2rr$Time))
head(m.b2rr, header = T)

# Pull out wells over time
m.0 <-  m.b2rr[,c(1,2,12,22)]
m.10 <-  m.b2rr[,c(1,32,41,50)]
m.20 <-  m.b2rr[,c(1,3,13,23)]
m.30 <-  m.b2rr[,c(1,33,42,51)]
m.40 <-  m.b2rr[,c(1,4,14,24)] 
m.50 <-  m.b2rr[,c(1,34,43,52)]
m.60 <-  m.b2rr[,c(1,5,15,25)]
m.80 <-  m.b2rr[,c(1,35,44,53)]
m.100 <-  m.b2rr[,c(1,6,16,26)]
m.120 <-  m.b2rr[,c(1,36,45,54)]
m.140 <-  m.b2rr[,c(1,7,17,27)]
m.160 <-  m.b2rr[,c(1,37,46,55)]
m.200 <-  m.b2rr[,c(1,8,18,28)]
m.220 <-  m.b2rr[,c(1,38,47,56)]
m.240 <-  m.b2rr[,c(1,9,19,29)]
m.260 <-  m.b2rr[,c(1,39,48,57)]
m.280 <-  m.b2rr[,c(1,10,20,30)]
m.300 <-  m.b2rr[,c(1,40,49,58)]
jcvi <-  m.b2rr[,c(1,11,21,31)]


# Run Gompertz (only need to run once then comment out)

m.t0.2rr <- growth.modGomp(input = m.0, output.name = "m.t0.2rr.parms",
              synergy = F, temp = F, smooth = T, trim = T)
m.t10.2rr <- growth.modGomp(input = m.10, output.name = "m.t10.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t20.2rr  <- growth.modGomp(input = m.20, output.name = "m.t20.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t30.2rr  <- growth.modGomp(input = m.30, output.name = "m.t30.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t40.2rr  <- growth.modGomp(input = m.40, output.name = "m.t40.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t50.2rr  <- growth.modGomp(input = m.50, output.name = "m.t50.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t60.2rr  <- growth.modGomp(input = m.60, output.name = "m.t60.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t80.2rr  <- growth.modGomp(input = m.80, output.name = "m.t80.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t100.2rr  <- growth.modGomp(input = m.100, output.name = "m.t100.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t120.2rr  <- growth.modGomp(input = m.120, output.name = "m.t120.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t140.2rr  <- growth.modGomp(input = m.140, output.name = "m.t140.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t160.2rr  <- growth.modGomp(input = m.160, output.name = "m.t160.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t200.2rr  <- growth.modGomp(input = m.200, output.name = "m.t200.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t220.2rr  <- growth.modGomp(input = m.220, output.name = "m.t220.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t240.2rr  <- growth.modGomp(input = m.240, output.name = "m.t240.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t260.2rr  <- growth.modGomp(input = m.260, output.name = "m.t260.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t280.2rr  <- growth.modGomp(input = m.280, output.name = "m.t280.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.t300.2rr  <- growth.modGomp(input = m.300, output.name = "m.t300.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)
m.jcvi.t1000.2rr  <- growth.modGomp(input = jcvi, output.name = "m.jcvi.t1000.2rr.parms",
               synergy = F, temp = F, smooth = T, trim = T)

t0.out.m.2rr <- read.table("../output/m.t0.2rr.parms.txt", sep = ",", header=TRUE)
t10.out.m.2rr <- read.table("../output/m.t10.2rr.parms.txt", sep = ",", header=TRUE)
t20.out.m.2rr <- read.table("../output/m.t20.2rr.parms.txt", sep = ",", header=TRUE)
t30.out.m.2rr <- read.table("../output/m.t30.2rr.parms.txt", sep = ",", header=TRUE)
t40.out.m.2rr <- read.table("../output/m.t40.2rr.parms.txt", sep = ",", header=TRUE)
t50.out.m.2rr <- read.table("../output/m.t50.2rr.parms.txt", sep = ",", header=TRUE)
t60.out.m.2rr <- read.table("../output/m.t60.2rr.parms.txt", sep = ",", header=TRUE)
t80.out.m.2rr <- read.table("../output/m.t80.2rr.parms.txt", sep = ",", header=TRUE)
t100.out.m.2rr <- read.table("../output/m.t100.2rr.parms.txt", sep = ",", header=TRUE)
t120.out.m.2rr <- read.table("../output/m.t120.2rr.parms.txt", sep = ",", header=TRUE)
t140.out.m.2rr <- read.table("../output/m.t140.2rr.parms.txt", sep = ",", header=TRUE)
t160.out.m.2rr <- read.table("../output/m.t160.2rr.parms.txt", sep = ",", header=TRUE)
t200.out.m.2rr <- read.table("../output/m.t200.2rr.parms.txt", sep = ",", header=TRUE)
t220.out.m.2rr <- read.table("../output/m.t220.2rr.parms.txt", sep = ",", header=TRUE)
t240.out.m.2rr <- read.table("../output/m.t240.2rr.parms.txt", sep = ",", header=TRUE)
t260.out.m.2rr <- read.table("../output/m.t260.2rr.parms.txt", sep = ",", header=TRUE)
t280.out.m.2rr <- read.table("../output/m.t280.2rr.parms.txt", sep = ",", header=TRUE)
t300.out.m.2rr <- read.table("../output/m.t300.2rr.parms.txt", sep = ",", header=TRUE)
t1000.out.m.2rr <- read.table("../output/m.jcvi.t1000.2rr.parms.txt", sep = ",", header=TRUE)

treats.m.2rr <- read.csv("../data/treatments_batch2_rerun_M.csv")
treats.m.2rr[rep(seq_len(nrow(treats.m.2rr)), each = 18), ]


# M: replicate population 3 (plate 3)
...
# m.10 <-  m.b3[,c(1,32,42,52)]
# m.20 <-  m.b3[,c(1,3,13,23)]
# m.30 <-  m.b3[,c(1,33,43,53)]
# m.40 <-  m.b3[,c(1,4,14,24)] 
# m.50 <-  m.b3[,c(1,34,44,54)]
# m.60 <-  m.b3[,c(1,5,15,25)]
# m.80 <-  m.b3[,c(1,35,45,55)]
# m.100 <-  m.b3[,c(1,6,16,26)]
# m.120 <-  m.b3[,c(1,36,46,56)]
# m.140 <-  m.b3[,c(1,7,17,27)]
# m.160 <-  m.b3[,c(1,37,47,57)]
# m.200 <-  m.b3[,c(1,8,18,28)]
# m.220 <-  m.b3[,c(1,38,48,58)]
# m.240 <-  m.b3[,c(1,9,19,29)]
# m.260 <-  m.b3[,c(1,39,49,59)]
# m.280 <-  m.b3[,c(1,10,20,30)]
# m.300 <-  m.b3[,c(1,40,50,60)]
...
# M: replicate population 4  (plate 4)
...
# m.10 <-  m.b4[,c(1,32,42,52)]
# m.20 <-  m.b4[,c(1,3,13,23)]
# m.30 <-  m.b4[,c(1,33,43,53)]
# m.40 <-  m.b4[,c(1,4,14,24)] 
# m.50 <-  m.b4[,c(1,34,44,54)]
# m.60 <-  m.b4[,c(1,5,15,25)]
# m.80 <-  m.b4[,c(1,35,45,55)]
# m.100 <-  m.b4[,c(1,6,16,26)]
# m.120 <-  m.b4[,c(1,36,46,56)]
# m.140 <-  m.b4[,c(1,7,17,27)]
# m.160 <-  m.b4[,c(1,37,47,57)]
# m.200 <-  m.b4[,c(1,8,18,28)]
# m.220 <-  m.b4[,c(1,38,48,58)]
# m.240 <-  m.b4[,c(1,9,19,29)]
# m.260 <-  m.b4[,c(1,39,49,59)]
# m.280 <-  m.b4[,c(1,10,20,30)]
# m.300 <-  m.b4[,c(1,40,50,60)]
...
treats.nm.1 <- treats.nm.1[mixedorder(treats.nm.1$sample),]

#nm.parms.1 <- cbind(treats.nm.1, nm.parms.ord.1)
nm.parms.1 <- cbind(treats.nm.1, nm.parms.raw.1)

# treats.nm.1$well == nm.parms.1$Curve
treats.nm.1$sample == nm.parms.1$Curve
gen <- round((1997/300)*nm.parms.1$day,0)
nm.parms.1 <-cbind(nm.parms.1, gen)
nm.parms.1 <- nm.parms.1[, c(1:4,9:11,19)]
...
treats.nm.2 <- treats.nm.2[mixedorder(treats.nm.2$sample),]

#nm.parms.2 <- cbind(treats.nm.2, nm.parms.ord.2)
nm.parms.2 <- cbind(treats.nm.2, nm.parms.raw.2)

treats.nm.2$well == nm.parms.2$Curve
treats.nm.2$sample == nm.parms.2$Curve

gen <- round((1997/300)*nm.parms.2$day,0)
nm.parms.2 <-cbind(nm.parms.2, gen)
nm.parms.2 <- nm.parms.2[, c(1:4,9:11,19)]
...
treats.nm.3 <- treats.nm.3[mixedorder(treats.nm.3$sample),]

nm.parms.3 <- cbind(treats.nm.3, nm.parms.raw.3)

treats.nm.3$sample == nm.parms.3$Curve
gen <- round((1997/300)*nm.parms.3$day,0)
nm.parms.3 <-cbind(nm.parms.3, gen)
nm.parms.3 <- nm.parms.3[, c(1:4,9:11,19)]
...
nm.parms.raw.4 <- rbind(t0.out.nm.4, t10.out.nm.4, t20.out.nm.4, t30.out.nm.4, 
                  ...
                  t120.out.nm.4, t140.out.nm.4, t160.out.nm.4, t200.out.nm.4, t220.out.nm.4, 
                  t240.out.nm.4, t260.out.nm.4, t280.out.nm.4, t300.out.nm.4, t1000.out.nm.4)
#nm.parms.ord.4 <- nm.parms.raw.4[mixedorder(nm.parms.raw.4$Curve),]
treats.nm.4 <- treats.nm.4[mixedorder(treats.nm.4$sample),]

nm.parms.4 <- cbind(treats.nm.4, nm.parms.raw.4)
treats.nm.4$sample == nm.parms.4$Curve
gen <- round((1997/300)*nm.parms.4$day,0)
nm.parms.4 <-cbind(nm.parms.4, gen)
nm.parms.4 <- nm.parms.4[, c(1:4,9:11,19)]
...
treats.m.1 <- treats.m.1[mixedorder(treats.m.1$sample),]

m.parms.1 <- cbind(treats.m.1, m.parms.raw.1)
treats.m.1$sample == m.parms.1$Curve
gen <- round((1997/300)*m.parms.1$day,0)
m.parms.1 <-cbind(m.parms.1, gen)
m.parms.1 <- m.parms.1[, c(1:4,9:11,19)]


####
m.parms.raw.2 <- rbind(t0.out.m.2, t10.out.m.2, t20.out.m.2, t30.out.m.2, 
                  t50.out.m.2, t60.out.m.2, t80.out.m.2, t100.out.m.2, 
...
treats.m.2 <- treats.m.2[mixedorder(treats.m.2$sample),]

m.parms.2 <- cbind(treats.m.2, m.parms.raw.2)
treats.m.2$sample == m.parms.2$Curve
gen <- round((1997/300)*m.parms.2$day,0)
m.parms.2 <-cbind(m.parms.2, gen)
m.parms.2 <- m.parms.2[, c(1:4,9:11,19)]



####
m.parms.raw.2rr <- rbind(t0.out.m.2rr, t10.out.m.2rr, t20.out.m.2rr, t30.out.m.2rr,
                  t40.out.m.2rr, t50.out.m.2rr, t60.out.m.2rr, t80.out.m.2rr, t100.out.m.2rr, 
                  t120.out.m.2rr, t140.out.m.2rr, t160.out.m.2rr, t200.out.m.2rr, 
                  t220.out.m.2rr, t240.out.m.2rr, t260.out.m.2rr, t280.out.m.2rr,
                  t300.out.m.2rr, t1000.out.m.2rr)
#m.parms.ord.2rr <- m.parms.raw.2rr[mixedorder(m.parms.raw.2rr$Curve),]
treats.m.2rr <- treats.m.2rr[mixedorder(treats.m.2rr$sample),]

m.parms.2rr <- cbind(treats.m.2rr, m.parms.raw.2rr)
treats.m.2rr$sample == m.parms.2rr$Curve
gen <- round((1997/300)*m.parms.2rr$day,0)
m.parms.2rr <-cbind(m.parms.2rr, gen)
m.parms.2rr <- m.parms.2rr %>%
    filter(day != "0")
m.parms.2rr <- m.parms.2rr[, c(1:4,9:11,19)]
...
treats.m.3 <- treats.m.3[mixedorder(treats.m.3$sample),]

m.parms.3 <- cbind(treats.m.3, m.parms.raw.3)
treats.m.3$sample == m.parms.3$Curve
gen <- round((1997/300)*m.parms.3$day,0)
m.parms.3 <-cbind(m.parms.3, gen)
m.parms.3 <- m.parms.3[, c(1:4,9:11,19)]
...
treats.m.4 <- treats.m.4[mixedorder(treats.m.4$sample),]

m.parms.4 <- cbind(treats.m.4, m.parms.raw.4)
treats.m.4$sample == m.parms.4$Curve
gen <- round((1997/300)*m.parms.4$day,0)
m.parms.4 <-cbind(m.parms.4, gen)
m.parms.4 <- m.parms.4 %>%
    filter(day != "0")
m.parms.4 <- m.parms.4[, c(1:4,9:11,19)]


parms <- rbind(nm.parms.1, nm.parms.2, nm.parms.3, nm.parms.4,
               m.parms.1, m.parms.2, m.parms.2rr, m.parms.3, m.parms.4)
```
...
# Potential outliers
#ts <- ts %>% 
#  filter(cell != "m" | biorep != "6" | day != "160") %>%
#  filter(cell != "m" | biorep != "6" | day != "300") %>%
#  filter(cell != "m" | biorep != "7" | day != "280") %>%
#  filter(cell != "m" | biorep != "8" | day != "140")

# Non-minimal cells
ts.nm <- filter(ts, cell == c("nm")) %>%
  dplyr::select(day, gen, cell, biorep, A, umax, lag)

# Non-minimal cells by bioreplicate
ts.nm.b1 <- filter(ts.nm, biorep == c("1")) %>%
  dplyr::select(day, gen, cell, biorep, A, umax, lag)
ts.nm.b2 <- filter(ts.nm, biorep == c("2")) %>%
  dplyr::select(day, gen, cell, biorep, A, umax, lag)
ts.nm.b3 <- filter(ts.nm, biorep == c("3")) %>%
  dplyr::select(day, gen, cell, biorep, A, umax, lag)
...
#mod2 <- lme(umax ~ (day * cell), random = ~ 1 | biorep, data = data) # random intercept
#mod3 <- lme(umax ~ (day * cell), random = ~1 + cell | biorep, data = data) # random slope,intercept
#mod4 <- lme(umax ~ day, random = ~1 + day | biorep, data = data) # random intercept and slope
#AIC(mod1,mod2,mod3)
mod3 <- lme(umax ~ gen * cell, random = ~ 1 | biorep, data = data) # random intercept
r.squaredGLMM(mod3) # m = marginal (fixed); c = conditional (fixed and random)



# Parameters
nm.intercept  <- mod3$coefficients[1]$fixed[1] + mod3$coefficients[1]$fixed[3]
nm.slope <- mod3$coefficients[1]$fixed[2] + mod3$coefficients[1]$fixed[4]

m.intercept <- mod3$coefficients[1]$fixed[1]
m.slope <- mod3$coefficients[1]$fixed[2]

# https://aosmith.rbind.io/2018/11/16/plot-fitted-lines/
# new data for plotting
newdat.lme <- data.frame(day = data$day, gen = data$gen, cell = data$cell, biorep = data$biorep)
newdat.lme$predlme = predict(mod3, newdata = newdat.lme, level = 0)

# Get model matrix using prediction data set
des = model.matrix(formula(mod3)[-2], newdat.lme)
...
predvar = diag(des %*% vcov(mod3) %*% t(des))
...
per.delta.umax.nm <- (max(nm.lme$predlme) - min(nm.lme$predlme))/min(nm.lme$predlme) * 100
per.delta.umax.m <- (max(m.lme$predlme) - min(m.lme$predlme))/min(m.lme$predlme) * 100
...
# add points back
points(jitter(ts.m$gen,1), ts.m$umax, pch = 21, 
       bg = "red", col = "red", lwd = 2, cex = 1)
points(jitter(ts.nm$gen,1), ts.nm$umax, pch = 21, 
...
#text(75, 0.2, labels = "A", cex = 2)

# Add strain identifier

#grid.rect(unit(0.8, "npc"), unit(0.62, "npc"),
#          width = unit(0.15, "npc"), height = unit(0.05, "npc"), default.units = "npc",
#          gp=gpar(col = "white"))

# white box for minimal text
#grid.rect(unit(0.8, "npc"), unit(0.49, "npc"),
#          width = unit(0.15, "npc"), height = unit(0.05, "npc"), default.units = "npc",
#          gp=gpar(col = "white"))


#grid.text("Non-minimal", x = 0.72, y = 0.62, 
#            gp = gpar(fontsize = 12, col = "black"))

#grid.text("Minimal", x = 0.72, y = 0.49, 
#            gp = gpar(fontsize = 12, col = "black"))


#legend(1200, 0.26, c("Non-minimal", "Minimal"), pch = 21, pt.bg = c("blue", "red"),
#       pt.cex = 1, col = c("blue", "red"), pt.lwd = 2, bty = 'n', y.intersp = 1, 
#       lwd = 0)
...
plot(jitter(ts.nm$gen,1), ts.nm$A, xlim = c(-10, 2010), 
     ylim = c(-0.1, 1.1), type = "p", pch = 21, bg = "white", 
     col = "white", cex = 1.25, ylab = "", xlab = "", 
     cex.lab = 1.25, las = 1, lwd = 2, yaxt = "n", xaxt = "n")
box(lwd=2)

points(jitter(ts.m$gen,1), ts.m$A, pch = 22, 
       bg = "white", col = "white", lwd = 2, cex = 1.25)
...
# add points back
points(jitter(ts.m$gen,1), ts.m$A, pch = 21, 
       bg = "red", col = "red", lwd = 2, cex = 1)
points(jitter(ts.nm$gen,1), ts.nm$A, pch = 21, 
       bg = "blue", col = "blue", lwd = 2, cex = 1)
...
mtext(expression('Time (generations)'), side = 1,
     outer = TRUE, cex = 1.5, line = -3.5, adj = 0.5)
mtext(expression('Yield (absorbance)'), side = 2, outer = TRUE, 
      cex = 1.5, line = -2.5, adj = 0.55)
```
...
# add points back
points(jitter(ts.m$gen,1), ts.m$lag, pch = 21, 
       bg = "red", col = "red", lwd = 2, cex = 1)
points(jitter(ts.nm$gen,1), ts.nm$lag, pch = 21, 
       bg = "blue", col = "blue", lwd = 2, cex = 1)
...
# Add axes and labels
axis(side = 2, lwd.ticks = 2, cex.axis = 1.25, las = 1,
     labels = c("0.0", "10", "10"), at = c(0.0, 10, 20))
axis(side = 4, lwd.ticks = 2, cex.axis = 1.25, las = 1,
...
#par(mar = c(3, 6.5, 2.5, 1), oma = c(9.2, 1, 1.5, 1))
#par(mar = c(2, 4, 4, 2), oma = c(2, 0, 0, 0))

#par(mar = c(6, 6.5, 2.5, 1))
#par(mfrow=c(1,3), mai = c(1, 0.1, 0.1, 0.1))
#par(mfrow=c(2,1), mai = c(0.25, 0.1, 0.3, 0.1))
...
# 
# # plot confidence intervals for lme model
# polygon(c(nm.lme$gen, rev(nm.lme$gen)), c(nm.lme$upper, rev(nm.lme$lower)), 
#         col = "lightblue", border = NA)
# polygon(c(m.lme$gen, rev(m.lme$gen)), c(m.lme$upper, rev(m.lme$lower)), 
#         col = "pink", border = NA)
# 
# # add points back
# points(jitter(ts.m$gen,1), ts.m$umax, pch = 21, 
...
# 
# axis(side = 2, lwd.ticks = 2, cex.axis = 1.25, las = 1,
#      labels = c("0.00", "0.10", "0.20"), at = c(0.0,0.1, 0.20))
# axis(side = 4, lwd.ticks = 2, cex.axis = 1.25, las = 1,
#      at=c(0, 0.1, 0.20), labels = F)
# axis(side = 3, lwd.ticks = 2, cex.axis = 1.25, las = 1,
#      at = c(0, 500, 1000, 1500, 2000), labels = F)
...
points(jitter(ts.m.yield$gen,1), ts.m.yield$A, pch = 21, 
       bg = "red", col = "red", lwd = 2, cex = 1)
...
#mtext(expression('Maximum growth rate (d'^-1*')'), side = 2, outer = TRUE, 
#      cex = 1.5, line = -2.5, adj = 0.85)

#mtext(expression(paste('', mu ,'max (d'^-1*')')), 
#      side = 2, outer = TRUE, cex = 1.5, line = -2.5, adj = 0.9)

#mtext(expression(paste('', mu ,'max'[2]* 'd'^-1*')')), 
#      side = 2, outer = TRUE, cex = 1.5, line = -2.5, adj = 0.9)

#mtext(expression(paste('G, mu ,'max (d'^-1*')'), side = 2, outer = TRUE, 
#      cex = 1.5, line = -2.5, adj = 0.85)
