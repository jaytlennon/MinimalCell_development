...
# data.orig <- data # run this to remove outliers

# Maximum growth rate (umax)

# random intercept
# mod.umax <- lme(umax ~ gen * cell, random = ~ 1 | biorep, data = data.orig) 
# m = marginal (fixed); c = conditional (fixed and random)
# r.squaredGLMM(mod.umax) 
# summary(mod.umax)

# pred.umax <- as.vector(mod.umax$fitted[,1])
# resid.umax <- data.orig$umax - pred.umax
# rmse.umax <- sqrt(mean(resid.umax1)^2) # 3.704449e-05
# outlier.umax <- sum(abs(resid.umax-mean(resid.umax))/sd(resid.umax)>3)  # 4
# who.outliers <- which(abs(resid.umax-mean(resid.umax))/sd(resid.umax)>3) 
...
intercept.percent <- m.intercept / nm.intercept * 100
# 43%
...
mod <- lm(diag.umax.m$pred.umax ~ diag.umax.m$resid.umax)
newx <- seq(-0.05, 0.09, by = ((0.05- -0.09)/(length(diag.umax.m$pred.umax)-1)))
newy <- predict(mod, data.frame(x = newx))

plot(diag.umax.m$resid.umax, diag.umax.m$pred.umax, xlab = "Predicted values", 
     ylab = "Residuals", las = 1, xlim = c(-0.06, 0.1), 
      ylim = c(0.04, 0.16), type = "p", pch = 21, bg = "red", 
      col = "red", cex = 1.25, cex.lab = 1.25, las = 1, lwd = 2)
 box(lwd=2)
lines(newx, newy, col = "red", lty = 2, lwd = 1.5)

mod <- lm(diag.umax.nm$pred.umax ~ diag.umax.nm$resid.umax)
newx <- seq(-0.05, 0.05, by = ((0.05- -0.05)/(length(diag.umax.m$pred.umax)+2)))
newy <- predict(mod, data.frame(x = newx))
points(diag.umax.nm$resid.umax, diag.umax.nm$pred.umax, type = "p", pch = 21, bg = "blue",
       col = "blue", cex = 1.25)
lines(newx, newy, col = "blue", lty = 2, lwd = 1.5)

text(0.07, 0.135, "Non-minimal", col = "black", cex = 1)
text(0.08, 0.078, "Minimal", col = "black", cex = 1)
...
pred.yield <- mod.yield$fitted
resid.yield <- data$A - pred.yield
rmse.yield <- sqrt(mean(resid.yield)^2) # 5.507864e-05
outlier.yield <- sum(abs(resid.yield-mean(resid.yield))/sd(resid.yield)>3)  # 4 
who.outliers <- which(abs(resid.yield-mean(resid.yield))/sd(resid.yield)>3) # 48, 56, 80, 82
#      day  gen cell biorep         A       umax       lag
# 48   60  399    m      6    0.8028667 0.14263333  9.227133
# 56   80  533    m      6    0.8664000 0.14396667  7.765900
...
rmse.lag <- sqrt(mean(resid.lag)^2) # 9.913863e-05
outlier.lag <- sum(abs(resid.lag-mean(resid.lag))/sd(resid.lag)>3)  # 4 
who.outliers <- which(abs(resid.lag-mean(resid.lag))/sd(resid.lag)>3) # 2, 64, 72, 104
