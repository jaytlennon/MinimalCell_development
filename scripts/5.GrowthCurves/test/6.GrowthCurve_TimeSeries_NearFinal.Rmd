...
nm.b1 <- read.csv("../data/retry/batch_NM_1.csv")
...
treats.nm.1 <- read.csv("../data/retry/treatments_batch1_NM.csv")
treats.nm.1[rep(seq_len(nrow(treats.nm.1)), each = 19), ]


# NM: bioreplicate 2 plate

nm.b2 <- read.csv("../data/retry/batch_NM_2.csv")
...
treats.nm.2 <- read.csv("../data/retry/treatments_batch2_NM.csv")
treats.nm.2[rep(seq_len(nrow(treats.nm.2)), each = 19), ]


# NM: bioreplicate 3 plate

nm.b3 <- read.csv("../data/retry/batch_NM_3.csv")
...
treats.nm.3 <- read.csv("../data/retry/treatments_batch3_NM.csv")
treats.nm.3[rep(seq_len(nrow(treats.nm.3)), each = 19), ]

# NM: bioreplicate 4 plate

nm.b4 <- read.csv("../data/retry/batch_NM_4.csv")
...
treats.nm.4 <- read.csv("../data/retry/treatments_batch4_NM.csv")
treats.nm.4[rep(seq_len(nrow(treats.nm.4)), each = 19), ]


# M: bioreplicate 1 plate

m.b1 <- read.csv("../data/retry/batch_M_1.csv")
...
treats.m.1 <- read.csv("../data/retry/treatments_batch1_m.csv")
treats.m.1[rep(seq_len(nrow(treats.m.1)), each = 19), ]


# M: bioreplicate 2 plate (T40, T60, T100 didn't grow)

m.b2 <- read.csv("../data/retry/batch_M_2.csv")
...
m.50 <-  m.b2[,c(1,34,43,52)]
#m.60 <-  m.b2[,c(1,5,15,25)]
m.80 <-  m.b2[,c(1,35,44,53)]
...
m.300 <-  m.b2[1:which(m.b2[,1] == 15.00), c(1,40,49,58)]
...
treats.m.2 <- read.csv("../data/retry/treatments_batch2_M.csv")
treats.m.2 <- treats.m.2 %>%
    filter(day != "40" & day != "60" & day != "100")
treats.m.2[rep(seq_len(nrow(treats.m.2)), each = 18), ]


# M: RERUN: bioreplicate 2 plate

m.b2rr <- read.csv("../data/retry/batch_M_2_rerun.csv")
...
treats.m.2rr <- read.csv("../data/retry/treatments_batch2_rerun_M.csv")
treats.m.2rr[rep(seq_len(nrow(treats.m.2rr)), each = 18), ]


# M: bioreplicate 3 plate

m.b3 <- read.csv("../data/retry/batch_m_3.csv")
...
treats.m.3 <- read.csv("../data/retry/treatments_batch3_M.csv")
treats.m.3[rep(seq_len(nrow(treats.m.3)), each = 19), ]


# M: bioreplicate 4 plate

m.b4 <- read.csv("../data/retry/batch_M_4.csv")
...
treats.m.4 <- read.csv("../data/retry/treatments_batch4_M.csv")
...
m.parms.raw.2 <- rbind(t0.out.m.2, t10.out.m.2, t20.out.m.2, t30.out.m.2, 
                  t50.out.m.2, t80.out.m.2, t120.out.m.2, t140.out.m.2, t160.out.m.2,
                  t200.out.m.2, t220.out.m.2, t240.out.m.2, t260.out.m.2, t280.out.m.2,
                  t300.out.m.2, t1000.out.m.2)

# t60.out.m.2 and t100.out.m.2 = no growth; didn't converge
...
m.parms.2rr <- m.parms.2rr %>%
    filter(day != "0")
m.parms.2rr <- m.parms.2rr[, c(1:4,10:12,20)]
...
parms <- rbind(nm.parms.1, nm.parms.2, nm.parms.3, nm.parms.4,
               m.parms.1, m.parms.2rr, m.parms.3, m.parms.4)
```
...
# do the three sets of statistics here: umax, yield, and lag
# Yield, appears to be six minimal outlier points; should be removed up front before stats
# Specifi these points
# Lag, there are is some variation in minimal cell, but not in any direction...no removal of outliers
...
#AIC(mod1,mod2,mod3)
mod2 <- lme(umax ~ gen * cell, random = ~ 1 | biorep, data = data) # random intercept
r.squaredGLMM(mod3) # m = marginal (fixed); c = conditional (fixed and random)
summary(mod2)

# parameters
nm.intercept  <- mod2$coefficients[1]$fixed[1] + mod2$coefficients[1]$fixed[3]
nm.slope <- mod2$coefficients[1]$fixed[2] + mod2$coefficients[1]$fixed[4]

m.intercept <- mod2$coefficients[1]$fixed[1]
m.slope <- mod2$coefficients[1]$fixed[2]
...
newdat.lme <- data.frame(day = data$day, gen = data$gen, cell = data$cell, 
              biorep = data$biorep)
newdat.lme$predlme = predict(mod2, newdata = newdat.lme, level = 0)

# get model matrix using prediction data set
des = model.matrix(formula(mod2)[-2], newdat.lme)
...
predvar = diag(des %*% vcov(mod2) %*% t(des))
