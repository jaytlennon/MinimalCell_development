...
growth.modGomp <- function(input = " ", output.name = " ", skip = "", 
  ...
  for(i in 1:length(samples)){
    ...
    #best.f1<-fit1$res.mod[[best]] # original
    best.f1<<-fit1$res.mod[[best]] # Schoolmaster's suggestion
 ...
 #   abline(h = 0, lty = 3, lwd = 2)
    ...
    cfs<-coef(best.f1)
    resids <-residuals(best.f1$res.mod[[1]])
    #resids <-residuals(best.f1)
    ...
    #results$RSME[i] <- sqrt(mean(residuals(fit1$res.mod[[1]]))^2)
