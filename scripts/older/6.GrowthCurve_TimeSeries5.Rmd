...
plot(jitter(ts.nm$gen,1), ts.nm$umax, xlim = c(-10, 2200), 
...
grid.text("Non-minimal", x = 0.79, y = 0.62, 
            gp = gpar(fontsize = 12, col = "black"))

grid.text("Minimal", x = 0.77, y = 0.49, 
            gp = gpar(fontsize = 12, col = "black"))
...
# Three panel figure
```{r}
png(filename="./../../../figures/Fig2.GC.png",
    width = 800, height = 1600, res = 96*2)

plot.new()
layout(matrix(c(1:3), byrow = T))
par(mar = c(1, 6.5, 0.5, 1), oma = c(9.2, 2, 1.5, 1))
...
mtext(expression('Lag time (hrs)'), side = 2, outer = TRUE, 
      cex = 1.5, line = -2.5, adj = 0.5)

mtext(expression('Yield (a'[415]*')'), side = 2, outer = TRUE, 
      cex = 1.5, line = -2.5, adj = 0.1)

mtext(expression('Time (generations)'), side = 1,
     outer = TRUE, cex = 1.5, line = 4, adj = 0.7)
...
# Show Plot
img <- readPNG("./../../../figures/Fig2.GC.png")
grid.raster(img)
```
