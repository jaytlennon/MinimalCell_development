...
mydf<-read_csv("data2/cases_Mm.300.csv")
...
### black and white version of dN/dS figure
...
### blue and red version of dN/dS figure
brcols<-c("blue","red")
dnds <- ggplot(mydf, aes(x=Strain, y=dNdS_gdtools))
...
### Test whether dD/dS is different from 1?
mydnds<- read.csv("data2/case.Mm.dNdS_gdtools.csv",header=T)
...
idff<-read.csv("data2/mean.SE_Mm.300.csv")
BANC<-idff$W_anc1.0[2] / idff$W_anc.own[2]


myt <- read.csv("data2/cases_Mm.300.csv",header=T)
...
### Relative fitness (grey and pink bars....confusing) ###
...
### Relative fitness (blue and red....older...confusing) ###
brcols<-c("blue","red")
...
### Relative fitness plot currently (31 July 20) used in manuscript ###
### But with legend
interaction_data<-read_csv("data2/Interaction_data.csv")
...
### Relative fitness plot currently (31 July 20) used in manuscript without legend ###
...
mjdf<-read_csv("data2/cases_microbeJ.csv")
#mydf<-read_csv("C:\\Users\\rmoge\\Box Sync\\Mycoplasma\\Experiments\\data\\cases_Mm.300_1.2.csv")
#mydf$Strain <- factor(mydf$Strain, levels=c("Wildtype","Minimal"))
