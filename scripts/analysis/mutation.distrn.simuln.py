...
#%%
'''def locustag_to_gene(gb_record, feature_type, qualifier, qualifier_translation) :
    answer = dict()
    for (index, feature) in enumerate(gb_record.features) :
        if feature.type==feature_type :
            try:
                mygg=feature.qualifiers[qualifier_translation]
            except KeyError:
                mygg=feature.qualifiers[qualifier]
            if qualifier in feature.qualifiers:
                for value in feature.qualifiers[qualifier] :
                    if value in answer :
                        print("WARNING - Duplicate key %s for %s features %s and %s" \
                           % (value, feature_type, answer[value], index))
                    else :
                        answer[value] = mygg[0]
    return answer

gene_names = locustag_to_gene(myrec, "gene", "locus_tag", "gene")
#%%
########In this chunk, I had TWO SEPARATE functions with which I built a dict of dicts. I think it may make more sense to build the dict all in one loop, rather than piping an updated dict from one function to the next, though. I attempt to do that in the NEXT chunk.
...
tag_indexes = index_genbank_features(myrec, "gene", "locus_tag")
...
def locustag_to_gene(gb_record, feature_type, qualifier, qualifier_translation, in_dict) :
    updated = copy.deepcopy(in_dict)
    for (index, feature) in enumerate(gb_record.features) :
        if feature.type==feature_type :
            try:
                mygg=feature.qualifiers[qualifier_translation]
            ...
            if qualifier in feature.qualifiers:
                for value in feature.qualifiers[qualifier] :
                    updated[value]['gene'] = mygg[0]
    return updated

DD = locustag_to_gene(myrec, "gene", "locus_tag", "gene", tag_indexes)'''
...
p_3b_100000.to_csv(index=True,path_or_buf=r"C:\Users\rmoge\Box Sync\Mycoplasma\Strains\pvals_simulations_syn3B_100000.csv")
p_s1_100000.to_csv(index=True,path_or_buf=r"C:\Users\rmoge\Box Sync\Mycoplasma\Strains\pvals_simulations_syn1.0_100000.csv")
#%%
